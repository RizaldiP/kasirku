...
<div class="mb-6 p-6 rounded-2xl" style="background: linear-gradient(135deg, #7c3aed, #db2777);">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">{{ __('Add Member') }}</h1>
            <p class="text-purple-200 mt-1 text-sm">{{ __('Register New Member') }}</p>
        </div>
    </div>
</div>
...
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 card-hover">
        <form method="POST" action="{{ route('members.store') }}">
            ...
            <button type="submit" class="w-full py-3 gradient-purple text-white font-bold rounded-xl btn-scale shadow-md">
                <i class="bi bi-person-plus me-1"></i> {{ __('Save') }}
            </button>
        </form>
    </div>
</div>
