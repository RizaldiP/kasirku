...
@section('content')
    <div class="flex items-center gap-3 mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-200">{{ __('Edit') }} {{ __('Product') }}</h1>
    </div>
            ...
            <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        @if ($product->image)
                            <div class="mb-2">
                                <div class="w-20 h-20 bg-gray-100 dark:bg-slate-700 bg-cover bg-center bg-no-repeat rounded-lg ring-1 ring-gray-200 dark:ring-slate-600" style="background-image: url('{{ asset('storage/' . $product->image) }}')"></div>
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <input type="number" name="selling_price" value="{{ old('selling_price', $product->selling_price) }}" step="0.01" required
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1 block">{{ __('Buy Price') }}</label>
                        <input type="number" name="purchase_price" value="{{ old('purchase_price', $product->purchase_price) }}" step="0.01" required
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
                    </div>
                    <div>
