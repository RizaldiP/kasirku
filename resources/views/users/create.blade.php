...
@section('content')
  <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-200 mb-4">{{ __('Add User') }}</h1>
      ...
      <form action="{{ route('users.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Email') }}</label>
            <input type="email" name="email" value="{{ old('email') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Password') }}</label>
            <input type="password" name="password" required min="6" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
          </div>
          <div>
            ...
            <select name="role" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus appearance-none bg-white dark:bg-slate-800">
              <option value="kasir" {{ old('role') === 'kasir' ? 'selected' : '' }}>{{ __('Cashier') }}</option>
