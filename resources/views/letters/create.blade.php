...
@section('title', __('Create Letter'))

@section('content')
<div class="max-w-3xl mx-auto">
  <div class="flex items-center justify-between mb-4">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-200">{{ __('Create Letter') }}</h1>
  ...
  <form action="{{ route('letters.store') }}" method="POST" class="space-y-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 card-hover p-5 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" name="letter_number" value="{{ old('letter_number', $suggestedNumber) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Date') }} <span class="text-red-500">*</span></label>
          <input type="date" name="date" value="{{ old('date', date('Y-m-d')) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Attachment') }}</label>
          <input type="text" name="attachment_count" value="{{ old('attachment_count', '1 (satu) berkas') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('e.g. 1 (satu) berkas') }}">
        </div>
      </div>
      ...
      <div>
        ...
        <input type="text" name="subject" value="{{ old('subject') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('Permohonan...') }}">
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 card-hover p-5 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="recipient_name" value="{{ old('recipient_name') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('Yth. Bapak/Ibu...') }}">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Recipient Place') }}</label>
          <input type="text" name="recipient_place" value="{{ old('recipient_place', 'di tempat') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 card-hover p-5 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="sender_name" value="{{ old('sender_name', auth()->user()->name) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus">
        </div>
        <div>
          ...
          <input type="text" name="sender_position" value="{{ old('sender_position') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('e.g. Direktur') }}">
        </div>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Sender Address') }}</label>
        <textarea name="sender_address" rows="2" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('Address...') }}">{{ old('sender_address') }}</textarea>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 card-hover p-5 space-y-4">
      ...
      <div>
        ...
        <textarea name="body" rows="10" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus text-sm leading-relaxed" placeholder="{{ __('Write the body of the letter here...') }}">{{ old('body') }}</textarea>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ __('Place') }}</label>
          <input type="text" name="place" value="{{ old('place') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl input-focus" placeholder="{{ __('City name') }}">
        </div>
      </div>
    </div>

    <button type="submit" class="w-full py-2.5 px-4 gradient-primary text-white font-medium rounded-xl btn-scale">
      <i class="bi bi-save me-1"></i> {{ __('Save Letter') }}
    </button>
  </form>
</div>
@endsection